function [delta_tstar,delta_T,std_dtstar,pairwise,fmids] = combspectra(dat,fcross,samprate,parms,ifplot)
...
%     waitbar(count/N,hw)
    ...
    [ As,phis,wts ] = run_comb( dat(:,is1),dat(:,is2),fltinfo,wdo1,wdo2,jbds,dt,pretime,maxphi,corc_skip,fmin_cskip,ifplot==2 );
	...
	% re-norm wts
    wts(inds) = wts(inds)./rms(wts(inds)); % DANGER - if all wts zero, gives inf
    if any(isinf(wts))
        keyboard
    end
    ...
    [ dtstar,dT,A0,misfit,resnorm,A_SSE,P_SSE,lnA_SSE ] = invert_1pair_Aphi_4_dtdtstar( As(inds),phis(inds),fmids(inds), wts(inds),amp2phiwt,alpha);
    ...
    R2_lnA = 1 - A_SSE./(As(inds)'*diag(wts(inds))*As(inds));
